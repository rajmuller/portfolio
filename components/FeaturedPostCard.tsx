...
type FeaturedPostCardProps = {
  post: Exclude<PostType, undefined | null>;
};
...
const FeaturedPostCard = ({ post }: FeaturedPostCardProps) => (
  <div className="relative h-72">
    ...
    <div className="absolute flex h-full w-full flex-col items-center justify-center rounded-lg p-4">
      ...
      <p className="text-shadow mb-4 text-center text-2xl font-semibold text-white line-clamp-3">
        {post.title}
      </p>
      <div className="absolute bottom-0 flex w-full items-center justify-center bg-black bg-opacity-50 py-2">
        <Image
          unoptimized
        ...
        />
        <p className="ml-2 inline  align-middle font-medium text-white">
          {post!.author!.name}
        </p>
      </div>
    </div>
    <Link href={`/post/${post.slug}`}>
      <a className="absolute h-full w-full cursor-pointer" />
    </Link>
  </div>
);
